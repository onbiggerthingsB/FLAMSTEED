...
| Wall time | **47.7 s** total; per-cutoff 7.7–9.4 s, of which 5.3–6.3 s is the fit — **MEASURED, NOT RE-DERIVABLE**: a wall clock is a property of the machine and the moment, not of the run, and `_VOLATILE` strips it from every hash for exactly that reason. Re-running this cell will not reproduce it and is not meant to. |
...
**The R1 body (§1–§10) and Addendum A stand unchanged**, as records of the run they describe — AND THIS ADDENDUM TAKES PRECEDENCE OVER THEIR PROSE WHEREVER THE TWO DISAGREE ABOUT THE PRESENT STATE (dated note, 2026-08-20, Codex review of `7b9d7d1`). Their numbers are six-season numbers and remain correct as the record of a six-season run; two of their SENTENCES about what the retrospective covers were written when six was all there was, and now read as false to anyone who does not reach this addendum. Pointer sentences have been added at both, so the precedence is discoverable from where the disagreement is rather than only from here: §2's *"the retrospective is six seasons, not seven"* is superseded by B (2023/24 is scored, the ledger holds 200 rows, and the bootstraps at MW6 and later have seven blocks), and §2 Hole 2's *"the refusals are confined to the opener"* is true only of the two seasons R1 could reach — B.3 documents a capability gap wider than R1 thought, and the season count per cutoff is what B states (MW0 = 5, MW3 = 6, MW6 onwards = 7). §2's Hole 1 remains the correct account of why R1 was a six-season run, and the remedy it named — *"an operator verifies those three rows against the league's published record, sets `verified: true`, and reruns R1 … the rerun costs the six cutoffs of 2023/24 and nothing else — roughly a minute of compute"* — is substantially what happened, at 47.7 seconds. One detail of it did not: R1 wrote *an operator verifies*, and what actually happened is that **the assistant did the verifying and the owner authorised the flip** (B.1). Whether that satisfies D16 is a question about D16, and it is recorded plainly here and in A5 rather than folded into the word "operator".
...
> **Superseded as a statement about the present, 2026-08-20 — see [Addendum B](#addendum-b--2026-08-20--202324-added-after-the-owner-authorised-adjustment-attestation).** This paragraph is the correct record of R1, and every number below it is a six-season number and stays one. It is no longer true of the retrospective as it now stands: the four 2023/24 adjustment rows were verified (amendment **A5**), 2023/24 was scored, and the ledger holds 200 rows. Addendum B takes precedence over this sentence wherever the two disagree; B's tables carry the seven-season figures and say at each cutoff how many seasons they rest on.
...
> **Read with [Addendum B](#addendum-b--2026-08-20--202324-added-after-the-owner-authorised-adjustment-attestation), 2026-08-20.** *"The refusals are confined to the opener"* is true of the two seasons R1 could reach and is not a statement about the whole grid. B.3 records a capability gap wider than R1 thought it was, and B states what the enlarged grid actually contains: **MW0 = 5 seasons, MW3 = 6, MW6 onwards = 7**. The MW0 refusals at 2019/20 and 2020/21 stand unchanged, and so does the observation in this paragraph about what trips the ceiling; what is superseded is any reading of it as a count over the retrospective as it now stands.
