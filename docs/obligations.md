...
| **standing** | Off-machine backup: `data/` (5.9G, incl. lock-attested paid odds captures — zero copies exist) and `~/Desktop/flamsteed-backups/` (the all-refs bundle + worktree docs corpus, currently on the SAME disk it protects). Blocked on the owner naming a destination; the copying is scripted work once one exists. | Owner names destination; assistant copies | Bundle created + restore-verified 2026-08-12. |
