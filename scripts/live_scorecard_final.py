...
HONESTY RULE (leads the report): n=104 live matches -> wide CIs. This
scorecard INFORMS the adoption meeting; it never auto-triggers a model change.
The favorite-overconfidence question was CLOSED 2026-06-25 on n=1473 held-out
...
production-fidelity fit per distinct match day (~34); the content-addressed
cache turns re-runs into HITs. Store: pure martj42@a16ff3e (single source —
NO duplicate rows, unaffected by review-v2 finding C2).

KO outcome semantics: martj42 records the ET-inclusive score, so an
extra-time win scores as a win and a shootout game scores as a draw —
consistent with the model's 1X2 including the draw-then-decided branch.
...
STORE_DIR = Path("data/stores/full_final")
OUT_MD = Path("reports/live_scorecard_final.md")
OUT_JSON = Path("reports/live_scorecard_final.json")


def live_matches(store) -> pd.DataFrame:
    asof = store.read("results", cutoff="2026-07-20T00:00:00Z").copy()
    ...
    m = m[(m["date"] >= "2026-06-11") & (m["date"] <= "2026-07-19")]
    m = m.dropna(subset=["home_score", "away_score"]).reset_index(drop=True)
    if len(m) != 104:
        raise ValueError(f"expected 104 scored matches, got {len(m)}")
    return m


def knockout_stage_map(matches: pd.DataFrame) -> dict:
    """(normalized day, frozenset(pair)) -> stage, by chronological rank.

    Stages never share a calendar day in the 2026 schedule, so date-order
    rank is exact: first 16 KO games are R32, next 8 R16, then 4 QF, 2 SF,
    third-place, final. Fail-loud if a stage boundary falls inside one day
    (would mean the rank method can no longer disambiguate)."""
    ko = (matches[matches["date"] > "2026-06-27"]
          .sort_values("date").reset_index(drop=True))
    names = (["R32"] * 16 + ["R16"] * 8 + ["QF"] * 4 + ["SF"] * 2
             + ["third", "final"])
    if len(ko) != len(names):
        raise ValueError(f"knockout count {len(ko)} != {len(names)}")
    for i in range(1, len(names)):
        if (names[i] != names[i - 1]
                and ko.loc[i, "date"].normalize()
                == ko.loc[i - 1, "date"].normalize()):
            raise ValueError(
                f"stage boundary {names[i-1]}->{names[i]} inside one day")
    return {(r["date"].normalize(),
             frozenset((str(r["home_team"]), str(r["away_team"])))): names[i]
            for i, r in ko.iterrows()}
...
def main() -> int:
    ...
    stage_map = knockout_stage_map(matches)
    ...
    for day, dm in matches.groupby(matches["date"].dt.normalize()):
        ...
        for r in rows:
            ...
            r["date"] = str(day.date())
            r["stage"] = stage_map.get(
                (day, frozenset((r["home"], r["away"]))), "group")
    ...
    tails = {k: {"pred": float(np.mean([r[k] for r in all_rows])),
             ...
             for k, g in [("p_marg_ge2", 2), ("p_marg_ge3", 3),
                          ("p_marg_ge4", 4)]}
    ko_rows = [r for r in all_rows if r["stage"] != "group"]
    ...
    def stage_line(label, rows_s):
        if not rows_s:
            return f"| {label} | 0 | - | - |"
        rps_s = np.mean([rps(r["probs"], r["outcome"]) for r in rows_s])
        fav_hits = sum(max(r["probs"], key=r["probs"].get) == r["outcome"]
                       for r in rows_s)
        return (f"| {label} | {len(rows_s)} | {rps_s:.5f} "
                f"| {fav_hits}/{len(rows_s)} |")
    ...
    md = [
        ...
        "",
        f"- matches scored: {n} (72 group + {len(ko_rows)} knockout); "
        f"fresh fits this run: {fits_missed}",
        ...
        *[f"| margin>={k[-1]} | {v['pred']:.3f} | {v['real']:.3f} |"
        ...
        stage_line("group", [r for r in all_rows if r["stage"] == "group"]),
        *[stage_line(s, [r for r in all_rows if r["stage"] == s])
          for s in ["R32", "R16", "QF", "SF", "third", "final"]],
        stage_line("all knockout", ko_rows),
    ]
